    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            if not stock.empty:
                row = stock.iloc[0]
                print(
                    f"  样例: {row['代码']} {row['名称']} ¥{row['最新价']} ({row['涨跌幅']:+.2f}%)"
                )
        else:
            print("  [!] 返回数据为空")
...
def test_akshare_kline():
    ...
    try:
        ...
        df = ak.stock_zh_a_daily(
            symbol="sz000001", start_date=start_date, end_date=end_date
        )
        elapsed = time.time() - start
        ...
        if df is not None and not df.empty:
            ...
            latest = df.iloc[-1]
            print(
                f"  最新: {latest['date']} 开:{latest['open']} 高:{latest['high']} 低:{latest['low']} 收:{latest['close']}"
            )
        else:
            print("  [!] 返回数据为空")
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            if not stock.empty:
                row = stock.iloc[0]
                print(
                    f"  样例: {row['代码']} {row['名称']} 综合得分:{row.get('综合得分', 'N/A')}"
                )
        else:
            print("  [!] 返回数据为空")
